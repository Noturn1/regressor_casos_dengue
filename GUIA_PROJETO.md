...
3. regressão com **CNN-LSTM** (padrão) ou **CNN2D** (alternativa).

A ideia é produzir uma estimativa coerente e avaliá-la com split temporal e comparação com baselines.
...
- `series_loader.py` ✅
- `scaler.py` ✅
- `window_builder.py` ✅ (trilha legada univariada, mantida para referência)
...
- `models/cnn2d.py` ✅ (Conv2D sobre janela transposta 4×9×1 — alternativa)
...
4. Preparar entrada conforme a arquitetura via `prepara_entrada`: CNN-LSTM recebe `(n, 9, 4)` direto; CNN2D recebe transposta `(n, 4, 9, 1)`.
5. Treinar o modelo via `models.seleciona_arquitetura` (ambas em fase única).
...
`prepara_entrada` recebe a matriz bruta 9×4:
- **CNN-LSTM:** passa direto como `(9, 4)`.
...
- `window_builder.py` — janelas centradas (trilha legada, exclui dia central).
...
- `test_cnn2d.py` — smoke tests da CNN2D (shape de saída, treino mínimo).
...
venv/bin/python -m dengue_tl.tune_runner --csv "data/Dados 2007-2024.csv" --arquitetura cnn2d --n-trials 50
```
